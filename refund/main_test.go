...
func test(t *testing.T, a *assert.Assertions, stoOrd models.StoredOrder) {
    ...
    for rows.Next() {
        ...
        a.True(storedOrder.Order.Payment.PaymentAmount == (stoOrd.Order.Payment.PaymentAmount)*-1, "Payment amount is not -originalAmount")
        a.True(storedOrder.Order.Payment.PaymentType == "Credit", "PaymentType should be equal to 'Credit'")
